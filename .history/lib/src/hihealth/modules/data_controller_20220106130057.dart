...
class DataController {
  ...
  static Future<DataController> init(List<HiHealthOption> hiHealthOptions) async {
    if (_instance != null) {
      _channel.invokeMethod("init", List<Map<String, dynamic>>.from(hiHealthOptions.map((e) => e.toMap())));
    }
    return _instance;
  }
  ...
  Future<SampleSet> readDailySummation(DataType dataType, int startTime, int endTime) async {
    Map<String, dynamic> callMap = {"dataType": dataType.toMap(), "startTime": startTime, "endTime": endTime};
    final resultMap = await _channel.invokeMethod("readDailySummation", callMap);
  ...
  Future<SampleSet> readTodaySummation(DataType dataType) async {
    final resultMap = await _channel.invokeMethod("readTodaySummation", dataType.toMap());
    return SampleSet.fromMap(Map<String, dynamic>.from(resultMap));
  }
