...
class Group {
  ...
  Group({this.startTime, this.endTime, this.activityRecord, this.activityType, this.sampleSets, this.groupType, this.hasMoreSample});
  ...
  factory Group.fromMap(Map<String, dynamic> map) {
    return Group(
        ...
        activityRecord: map['activityRecord'] != null ? ActivityRecord.fromMap(Map<String, dynamic>.from(map['activityRecord'])) : null,
        activityType: map['activityType'] != null ? map['activityType'] : null,
        sampleSets: map['sampleSets'] != null ? List<SampleSet>.from(map['sampleSets'].map((e) => SampleSet.fromMap(Map<String, dynamic>.from(e)))) : null,
        groupType: map['groupType'] != null ? GroupType.values[map['groupType'] + 1] : null,
        hasMoreSample: map['hasMoreSample'] != null ? map['hasMoreSample'] : null);
  }
  ...
  String? get getActivity => HiHealthActivities.typeActivityMap[this.activityType!];

  /// Checks whether the group has the same structure.
  bool hasSameSample(Group group) {
    return this.activityType == group.activityType && this.groupType == this.groupType && this.startTime == group.startTime && this.endTime == this.endTime;
  }

  @override
  bool operator ==(Object other) {
    if (!isTypeEqual(this, other)) return false;
    Group compare = other as Group;
    List<dynamic> currentArgs = [startTime, endTime, activityRecord, activityType, sampleSets, groupType, hasMoreSample];
    List<dynamic> otherArgs = [compare.startTime, compare.endTime, compare.activityRecord, compare.activityType, compare.sampleSets, compare.groupType, compare.hasMoreSample];
    return isEquals(this, other, currentArgs, otherArgs);
  }

  @override
  int get hashCode => hashValues(startTime, endTime, activityRecord, activityType, hashList(sampleSets), groupType, hasMoreSample);
}
